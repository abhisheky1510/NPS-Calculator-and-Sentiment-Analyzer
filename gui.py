...
def import_csv_data():
    ...
    print(csv_file_path)
    v.set(csv_file_path)
    dataset2 = pd.read_csv(csv_file_path)
    import pandas as pd
    import nltk
...
root = tk.Tk()
root.title('NPS Evaluator')
root.minsize(550, 300)
tk.Label(root, text='File Path').grid(row=0, column=0)
v = tk.StringVar()
entry = tk.Entry(root, textvariable=v).grid(row=0, column=1)
tk.Button(root, text='Browse Data Set',command=import_csv_data).grid(row=1, column=0, pady = 20, padx = 10)
tk.Button(root, text='Select',command=root.destroy).grid(row=1, column=1)
root.mainloop()
